...
void Setup(const LaunchOptions& options) {
  ...
  window_.create(sf::VideoMode({options.window_width, options.window_height}),
                 "SFML window");

  camera_.Setup(window_.getSize());

  tilemap_ptr_->Setup(options.tilemap_width, options.tilemap_height);
  tilemap_ptr_->set_camera(window_, camera_);
  tilemap_ptr_->on_released_left = []() {
    ...
    npc_manager_.Add(npc_adding_type, TileMap::tile_pos(sf::Vector2i(world)),
  ...
  btnBlue = btn_factory.CreateButton(sf::Vector2f(100.f, window_size.y - 100.f),
                                     "Blue");
  btnBlue->on_released_left = []() {
    npc_adding_type = api::ai::NpcType::kBlueWood;
  };

  btnRed = btn_factory.CreateButton(sf::Vector2f(200.f, window_size.y - 100.f),
                                    "Red");
  ...
  btnExit = btn_factory.CreateButton(sf::Vector2f(window_size.y - 30.f, 30.f),
                                     "Exit");
...
void SpawnInitialNpcs(size_t total) {
  ...
  const auto& walkables = tilemap_ptr_->walkables();
  if (walkables.empty()) {
  ...
  std::uniform_int_distribution<size_t> tile_dist(0, std::size(walkables) - 1);
