...
namespace graphics {
    void TilemapRenderer::add_tile(sf::Vector2f pos, sf::Vector2f offset, sf::FloatRect texBounds) {
    ...
    void TilemapRenderer::Setup(sf::Texture *texture, sf::Vector2f gridSize, sf::Vector2f gridOffset,
        ...
        for (float x = 0.f; x < gridSize.x; x += gridOffset.x) { // NOLINT(*-flp30-c)
            for (float y = 0.f; y < gridSize.y; y += gridOffset.x) { // NOLINT(*-flp30-c)
                ...
                add_tile({x, y}, gridOffset, texBounds);
            }
        }
    }
